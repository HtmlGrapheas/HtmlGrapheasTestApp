...
void spanner_wo(int y, int count, const FT_Span* spans, void* user)
{
  ...
  for (int i = 0; i < count; i++) {
    uint32_t color = ((spans[i].coverage / 2) << baton->rshift)
...
/*
  SDL_WM_SetCaption("\"Simple\" SDL+FreeType+HarfBuzz Example",
      "\"Simple\" SDL+FreeType+HarfBuzz Example");
...
//      stuffbaton.last_pixel = (uint32_t*) (((uint8_t*) sdl_surface->pixels)
//          + sdl_surface->pitch * sdl_surface->h);
...
//            stuffbaton.pixels = (uint32_t*) (((uint8_t*) sdl_surface->pixels)
//                                    + gy * sdl_surface->pitch)
//                + gx;
    ...
    while (SDL_PollEvent(&event)) {
      switch (event.type) {
        ...
        case SDL_VIDEORESIZE:
          ...
          screen =
              SDL_SetVideoMode(event.resize.w, event.resize.h, bpp, videoFlags);
          if (!screen) {
